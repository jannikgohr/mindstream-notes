...
const settings = vi.hoisted(() => ({
  ...
  get: vi.fn((id: string) => settings.values.get(id))
}));
...
describe('invalidation refreshes', () => {
  it('reloads word chars and language tags, and clears the cache', async () => {
    ...
    await vi.waitFor(() => expect(api.wordChars).toHaveBeenCalled());
  ...
  it('skips the word-chars call when no language is selected', async () => {
    ...
    const mod = await load();
    mod.invalidateDiagnostics();
    await vi.waitFor(() => expect(api.available).toHaveBeenCalled());
