<script lang="ts">
  import { ArrowUpRight, CornerUpLeft, LoaderCircle } from '@lucide/svelte';
  import NoteKindIcon from './NoteKindIcon.svelte';
  import { loadNoteRelations } from '$lib/notes/note-relations';
  ...
  let { noteId, kind }: Props = $props();
  let relationIds = $state<string[]>([]);
  ...
  const emptyLabel = $derived(
    kind === 'backlinks'
      ? tUi('sidebar.backlinks.empty')
      : tUi('sidebar.outgoingLinks.empty')
  );
  const notes = $derived(
    relationIds
    ...
    void loadNoteRelations(id, Object.values(tree.notesById))
      .then((relations) => {
        if (!cancelled && noteId === id) {
          relationIds =
            kind === 'backlinks' ? relations.backlinks : relations.outgoing;
        }
      })
      .catch((error) => {
...
>
  <h3
    class="mb-2 flex items-center gap-2 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground"
  >
    {#if kind === 'backlinks'}
  ...
  {#if loading && relationIds.length === 0}
