...
| D5 | Color picker + brush size | Toolbar additions in `ui/color_picker.rs` / `ui/brush.rs`. Most plumbing already in place: render thread tracks `current_stroke_color` + `current_stroke_packed` per stroke; wgpu pipeline takes per-stroke packed colour through `StrokesDoc::begin_stroke(color)`; front-buffer overlay receives colour + width range from Rust via `push_live_ink_style`. D5's work is the toolbar widget + the bridge from `RenderActions::set_color` / `set_brush_width` into render-thread state (replacing the current `color_for_input(tool, buttons)` debug-palette mapping). Per-stroke `width` field in the yrs schema is reserved for the brush size — wires up here.                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| D8 | Stylus button + gesture → action mapping | UX-policy layer over C6 + the discrete-gesture path. Maps `StylusButtons` bitmask states (held during a stroke) and discrete `StylusGesture` events (between strokes) onto canvas actions: switch to eraser, cycle colour, undo, open quick-picker, etc. Lives in `ui/` (above the render thread) because the policy needs to see the current tool mode + the user's settings — render thread stays a dumb carrier of the input bits. Apple Pencil 2 double-tap respects the system pref (`UIPencilInteraction.preferredTapAction`) when running on iOS so the gesture does the same thing in our app as in others; Android side has no equivalent system pref, so we expose our own setting. Depends on C6. **In progress** — first slice (F7's option (a) "barrel-button held → temporary Eraser") is wired: `render.rs::effective_tool_at_down` latches the modifier at every DOWN; a render-thread `stroke_tool_override: Option<ToolMode>` carries the latched value across MOVE / UP without flipping mid-stroke; cleared on every state-reset site (UP / Cancel / Clear / SetActiveNote / tool change). Toolbar's `UiState::current_tool` reflects the override during the stroke so the user gets visual feedback. Remaining: discrete `StylusGesture` shape for Apple Pencil 2 double-tap / Pencil Pro squeeze; the configurable mapping (`editor.ink.barrelButton` setting per F5/F7); cycle-colour / quick-picker bindings. |
