...
//!
//! Same architectural shape as the app's markdown yrs-doc facade
...
fn encode_payload_v2(color: u32, width: f32, points: &[f32], pressures: &[f32]) -> Vec<u8> {
    debug_assert_eq!(
        points.len() % 2,
        0,
        "points must be interleaved (x, y) pairs"
    );
    debug_assert_eq!(
        points.len() / 2,
        pressures.len(),
...
fn decode_geometry(body: &[u8], n_points: usize) -> Option<(Vec<f32>, Vec<f32>)> {
    ...
    for _ in 0..(n_points * 2) {
        points.push(f32::from_le_bytes(
            body[offset..offset + 4].try_into().ok()?,
        ));
        offset += 4;
    }
    for _ in 0..n_points {
        pressures.push(f32::from_le_bytes(
            body[offset..offset + 4].try_into().ok()?,
        ));
        offset += 4;
    }
...
mod tests {
    ...
    fn pressure_round_trips() {
        ...
        assert_eq!(
            ...
            vec![(vec![0.0, 0.0, 1.0, 1.0, 2.0, 2.0], vec![0.25, 0.50, 0.75],)]
        );
    }
