...
export function getCardShape(): CardShape {
  return {
    priority: true,
    progress: true,
    description: true,
...
export function getEditorItems(shape?: EditorShape | CardShape) {
  const s: EditorShape = shape ?? {
    description: true,
    priority: true,
    progress: true,
    deadline: true,
  ...
  if (s.priority) {
    const priorityData = configOf<CardPriorityShape>(s.priority)?.data;
    items.push({
      comp: 'richselect',
      key: 'priority',
      label: 'Priority',
  ...
  const tagsData = configOf<CardTagsShape>(s.tags)?.data;
  if (tagsData) {
    items.push({
      comp: 'multicombo',
      key: 'tags',
  ...
  const usersData = configOf<CardUsersShape>(s.users)?.data;
  if (usersData) {
    items.push({
      comp: 'multicombo',
      key: 'users',
